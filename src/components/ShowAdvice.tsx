    ...
    >
      ...
      <div className="w-full flex-1 pt-32 flex">
        ...
        <div className="w-full flex flex-col py-16 items-center space-y-5 max-[800px]:pt-32">
          <div className="text-white font-medium opacity-80 text-2xl text-center">
            HERE IS YOUR ADVICE
          </div>
          <div className="w-96 border-t-2 border-t-white opacity-10"></div>
          <div className="text-white font-bold text-xl pt-10 opacity-80 w-3/4 text-center">
            {advice?.advice}
          </div>
        </div>
      </div>
      <div
        className="absolute right-20 bottom-20 w-40 h-40 cursor-pointer flex justify-center max-[800px]:bottom-10 max-[800px]:right-20"
        onClick={handleSave}
      >
      ...
      <div
        className="absolute left-1/2 bottom-20 w-40 h-40 cursor-pointer flex justify-center items-center max-[800px]:bottom-10 max-[800px]:left-20"
        onClick={handleAnother}
      >
        <img className="absolute w-full h-full" src={image} alt="" />
        <div className="w-full text-white z-20 font-semibold text-xl text-center pr-5">
          ANOTHER ADVICE
        </div>
      </div>
    </section>
